#!/usr/bin/env bash
# overnight_etnaify.sh — drive the etna-ify pipeline over a list of Rust candidates,
# one candidate per `claude -p` headless invocation with a wall-clock cap.
#
# Behaviors:
#   * Skips candidates whose workloads/Rust/<name>/etna.toml already has [[tasks]].
...
# redo completed work.
#
# Order is execution order: cheap, high-confidence wins first so partial
# completion still yields something; expensive stretch goals last.
CANDIDATES=(
  "lru:3600"
  "strsim:3600"
  "num-rational:3600"
...
Follow the etna-ify pipeline verbatim as described in etna-ify/prompts/run.md and
etna-ify/AGENTS.md. Read both documents first.
...
state from an interrupted prior run:
  ...
  - etna.toml — each `[[tasks]]` group is a completed atomize iteration. Skip
    those candidates in atomize.
  - patches/*.patch — each corresponds to a patch-injection variant. If its
    name is in etna.toml, skip. (No per-variant git branches exist; the patch
    file is the durable variant artefact.)
...
unrelated repos. Use the standard marauders comments or patches/<variant>.patch for
mutation injection — no per-variant git branches.
