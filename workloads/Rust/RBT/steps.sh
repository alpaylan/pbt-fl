...
(cd ${workload_path} &&     crabcheck-profiling-analysis coverage rbt --print-json)

(cd ${workload_path} &&     rm -r coverage)

(cd ${workload_path} &&     rm -r jsondata)

(cd ${workload_path} &&     rm -r profdata)

(cd ${workload_path} &&     rm default*.profraw)
